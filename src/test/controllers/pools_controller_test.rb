...
class PoolsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pool" do
    assert_difference('Pool.count') do
      post pools_url, params: { pool: { income: @pool.income, name: @pool.name, type: @pool.type } }
    end
  ...
  test "should update pool" do
    patch pool_url(@pool), params: { pool: { income: @pool.income, name: @pool.name, type: @pool.type } }
